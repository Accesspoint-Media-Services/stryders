...
?><ul class="social-media__menu">
	<?php foreach ($args['items'] as $index => $media) : ?>
		<li class="social-media__item social-media--<?= esc_attr($media['icon']); ?>">
			<?php if (!empty($media['link']['url'])) : ?>
				...
				<?php endif; ?>
				<span class="sr"><?= esc_html($media['link']['title'] ?: $media['icon']); ?></span>
				<?php locate_template(sprintf('/images/icons/%s.svg', $media['icon']), true, false); ?>
				<?php if (!empty($media['link']['url'])) : ?>
				</a>
			<?php endif; ?>
		</li>
